...
from TurNuP.code.metabolite_preprocessing import *
from TurNuP.code.enzyme_representations import *
...
def kcat_predicton(substrates, products, enzymes, info):
    ...
    print("Step 3/3: Making predictions for kcat.",enzymes)
    #Merging the reaction and the enzyme DataFrame:
    df_kcat = pd.DataFrame(data = {"substrates" : substrates, "products" : products,
    							 "enzyme" : enzymes,"OriginAA" : info['OriginAA'],"site" : info['site'],"MutationAA" : info['MutationAA'], "index" : list(range(len(substrates)))})
	#"OriginAA" : info['OriginAA'],"site" : info['site'],"MutationAA" : info['MutationAA']
...
def predict_kcat(X):
	bst = pickle.load(open(join(CURRENT_DIR, "data", "data", "saved_models", "xgboost", "xgboost_train_and_test.pkl"), "rb"))
	dX = xgb.DMatrix(X)
